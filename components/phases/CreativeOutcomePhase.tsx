  ...
  return (
    <section className="space-y-10">
      <header className="space-y-3">
        <p className="text-xs font-semibold uppercase tracking-[0.2em] text-zinc-500">Your page</p>
        <h2 className="font-display text-3xl text-zinc-900 sm:text-4xl">A page shaped by today&apos;s weather inside</h2>
        <p className="max-w-2xl text-zinc-600">
          Typography and color follow the mood in your words, not a label on you. Threads that showed up today:{" "}
      ...
      <div className="rounded-2xl border border-zinc-200 bg-zinc-50/80 p-6">
        <p className="text-xs font-semibold uppercase tracking-[0.2em] text-zinc-500">Gentle snapshot</p>
        <dl className="mt-4 grid gap-4 sm:grid-cols-3">
          <div>
            <dt className="text-xs text-zinc-500">Outlook tint</dt>
            <dd className="text-2xl font-semibold text-zinc-900">
              {result.analysis.sentiment.valence.toFixed(2)}
            </dd>
          </div>
          <div>
            <dt className="text-xs text-zinc-500">Energy level in the words</dt>
            <dd className="text-2xl font-semibold text-zinc-900">
              {result.analysis.sentiment.arousal.toFixed(2)}
            </dd>
          </div>
          <div>
            <dt className="text-xs text-zinc-500">Safety check</dt>
